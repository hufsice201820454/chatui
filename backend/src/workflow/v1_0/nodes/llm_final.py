...
def _build_chat(temperature: float = 0.0) -> ChatOpenAI:
    """ChatOpenAI 인스턴스 팩토리."""
...
def llm_final(state: AgentState) -> AgentState:
    ...
    add_messages reducer: 새 HumanMessage·AIMessage만 반환해 누적.
    """
    ...
    if hitl_action == "reject":
        ...
        user_content = "\n".join(parts)
        chat = _build_chat(temperature=0.7)
        new_reject_count = reject_count + 1
    else:
        user_content = f"문의: {query}"
        chat = _build_chat(temperature=0.0)
        new_reject_count = reject_count      # 변경 없음

    invoke_messages = [HumanMessage(content=user_content)]

    try:
        response: AIMessage = chat.invoke(
            [SystemMessage(content=system_prompt)] + invoke_messages
        )
    ...
    return {
        **state,
        "messages": invoke_messages + [response],
